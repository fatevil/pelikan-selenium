...
public class DriverFactory {
    ...
    private void set(String driverName, String browserDir, String executable) {
        String propertyName = "webdriver." + driverName.toLowerCase() + ".driver";
        String driverLocation = System.getProperty(propertyName);
        if (driverLocation == null) {
            String osDir = operatingSystem.toLowerCase();
            if (osDir.indexOf("windows") > -1) {
                osDir = "windows";
                executable += ".exe";
            }
            else if (osDir.indexOf("mac") > -1) {
                osDir = "osx";
            }

            String bitsDir = "64bit";
            if (systemArchitecture.indexOf("32") > -1) {
                bitsDir = "32bit";
            }

            Path path = Paths.get(System.getProperty("buildDirectory"), "test-classes", "selenium_standalone_binaries", osDir, browserDir, bitsDir, executable);
            driverLocation = path.toString();
            System.out.println("Setting driver location to " + driverLocation);
            System.setProperty(propertyName, driverLocation);
        }
    }
    ...
    private void instantiateWebDriver(DriverType driverType) throws MalformedURLException {
        ...
        if (useRemoteWebDriver) {
            ...
            webDriver = driverType.getWebDriverObject(desiredCapabilities, (dt, a, b, c) -> set(a, b, c));
        }
